...
- ????????DNA????????????Pt???? `??Pt 1 = DNA 5` ?DNA??????
- ???????Pt????????DNA????????????????????
- ND10 QA?????Pt?????????????????????/??/????????????????????????

## ND10pre UI Follow-up

The ND08 release QA judgment remains pending final ND10 runtime QA after the
latest pre-QA UI changes.

Additional checks added:

- Locked new-dino select cards should display hero-derived silhouettes.
- Unknown research tab should show Spinosaurus plus all six new dino unlock cards.
- Unknown research cards should display DNA icon + value, matching other research cards.
- Home screen should use the regenerated production research-facility background.
- Home lower panel should remain non-interactive and reserved for future event display.

Reference:

- `docs/design/new_dinos_home_research_ui_fix_nd10pre.md`
