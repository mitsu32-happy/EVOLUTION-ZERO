...
## MVP-P07b save compatibility note

P07bでは、旧save相当の `companion` 未定義データを `SaveManager` に読み込ませ、以下を確認しました。
- companion default補完
- 既存DNA/研究Pt/チュートリアル状態の保持
- 卵付与
- 孵化開始
- 孵化完了
- お供セット
...
致命的なsave互換問題は確認されていません。
## MVP-P06b Save Compatibility Note

P06b adds `companion.eggDiscovered` for research tab reveal control.

- Missing `companion` still normalizes to the default companion state.
- `eggDiscovered` is derived as true for older saves that already have
  `eggPending`, `eggIncubating`, `lastHatchedId`, or owned companion IDs.
- `grantCompanionEgg`, incubation start, incubation completion, and debug grant
  paths mark `eggDiscovered` true.
