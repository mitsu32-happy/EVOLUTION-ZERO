...
```text
http://localhost:5179/EVOLUTION-ZERO/?debugIntroSeen=1&debugAutoPlay=1&debugInvincible=1&debugDino=ankylosaurus&debugForceEvolution=ankylosaurus_speed
```

ZERO進化を直接確認:

```text
http://localhost:5179/EVOLUTION-ZERO/?debugIntroSeen=1&debugAutoPlay=1&debugInvincible=1&debugDino=ornithomimus&debugForceEvolution=ornithomimus_zero&debugUnlockZeroEvolutions=1
```

図鑑を全確認:

```text
http://localhost:5179/EVOLUTION-ZERO/?debugIntroSeen=1&debugUnlockCodex=1&debugUnlockZeroEvolutions=1
```

## 実装メモ

- `debugFastEvolution` は `GameState.addSkill()` 内で適応スキル取得時だけ作用する。
...
1. `debugUnlockAllDinos=1` で新6体の選択画面表示を確認する。
2. `debugForceEvolution={dinoId}_{speed|hunting|attack|zero}` で24分岐のPlayScene表示を個別確認する。
3. `debugUnlockCodex=1&debugUnlockZeroEvolutions=1` で図鑑のbase/進化/ZERO表示を一括確認する。
4. `debugFastEvolution=1` で通常カード取得導線から進化候補UIを確認する。
