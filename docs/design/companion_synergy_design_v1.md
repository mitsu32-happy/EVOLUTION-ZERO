...
| `medic_saur` | アンキロサウルス | `ankylosaurus` | 将来予約 | 再生装甲 | 被弾時小回復、回復効果上昇 | 回復効果 +10%、低頻度小回復 | 相性: 未発見の恐竜 |
| `para_juvenile` | パラサウロロフス | `parasaurolophus` | 将来予約 | 共鳴探索 | EXP/pickup回収範囲上昇 | 回収範囲 +10% | 相性: 未発見の恐竜 |
| `stego_calf` | ステゴサウルス | `stegosaurus` | 将来予約 | 背板衝波 | 周囲範囲攻撃強化 | 範囲攻撃半径 +8%、低頻度小衝撃波 | 相性: 未発見の恐竜 |
| `ptera_chick` | プテラノドン | `pteranodon` | 将来予約 | 翼影支援 | 回避/遠距離支援強化 | 遠距離支援弾 +10%、または低確率回避補助 | 相性: 未発見の恐竜 |
| `compy_pack` | コンプソグナトゥス | `compsognathus` | 将来予約 | 群れの狩り | 低HP敵への追撃 | 低HP敵へのお供ダメージ +15% | 相性: 未発見の恐竜 |
| `exp_chaser` | オルニトミムス | `ornithomimus` | 将来予約 | 走査成長 | EXP獲得量/回収速度上昇 | EXP獲得 +6% または吸引速度 +12% | 相性: 未発見の恐竜 |
...
CS04では、共存シナジーを隠し要素にせず、既存のお供UIに表示した。

### 表示対象

- ホーム画面のお供パネル
  - CS05c以降、ホームではシナジーの `発動中` 表示を出さない。
  - ホーム表示恐竜は出撃時の最終選択とは限らないため、お供名・Lv・変更導線のみを表示する。
- ホーム画面のお供選択モーダル
  - enabled の所持お供カードに `共存シナジー: {相性恐竜}`、`効果: {shortLabel} / {発動中|未発動}` を表示。
  - enabled: false の将来予約お供は `相性: 未発見の恐竜` のみ表示。
- 研究画面のお供タブ 所持お供画面
  - 各所持お供行に `共存:{相性恐竜} 効果:{shortLabel} / {発動中|未発動}` を短縮表示。

### 未実装恐竜の扱い

- enabled: false の将来予約6組は通常UIで実名を出さず、`publicPlayerDinoName: '未発見の恐竜'` を表示する。
- enabled: false は通常UIでシナジー名、効果説明、`将来解放予定` 系の文言を表示しない。

### 表示文言

- 数値は出さず、`ボス特化`、`連撃`、`水弾支援`、`防御補助` などの短い説明を優先。
- 詳細仕様は `docs/design/companion_synergy_ui_cs04.md` に記録。

## CS05b / CS05c プレイ中表示メモ

CS05bでは、発動中の共存シナジーをPlaySceneの小型HUDとして表示した。
CS05cの実機確認でボスHPバーとの干渉が確認されたため、プレイ中の常時HUDは削除した。

- プレイ中効果は引き続き有効。
- PlaySceneには共存シナジーの常時HUDを表示しない。
- シナジー情報は、お供選択画面と研究画面のお供一覧で確認する。
- ホームのお供パネルにも `発動中` は表示しない。
- 詳細仕様は `docs/design/companion_synergy_display_cs05c.md` に記録。

## PF03 新6体接続メモ

新6体がmainへ実装済みになったため、PF03で将来予約だった6組を本接続した。

### enabled true 化した組み合わせ

| お供ID | プレイヤー恐竜ID | シナジーID | 効果 |
| --- | --- | --- | --- |
| `medic_saur` | `ankylosaurus` | `ankylo_regen_armor` | 回復効果 +10% |
| `para_juvenile` | `parasaurolophus` | `para_echo_scavenger` | 回収補助範囲 +10% |
| `stego_calf` | `stegosaurus` | `stego_plate_shock` | 範囲支援ダメージ +10% |
| `ptera_chick` | `pteranodon` | `pteranodon_wing_cover` | 遠距離支援ダメージ +10% |
| `compy_pack` | `compsognathus` | `compy_pack_hunt` | 低HP敵への支援ダメージ +12% |
| `exp_chaser` | `ornithomimus` | `ornithomimus_exp_runner` | EXP補助 +6% |

### UI方針変更

- 上記6組は通常UIで実装済み恐竜名を表示する。
- これらの組み合わせでは `未発見の恐竜` 表示を使わない。
- ホームのお供パネルはCS05c方針のまま、`発動中` 常時表示を出さない。

### セーブ変更

セーブ構造の変更はなし。シナジーは現在のプレイヤー恐竜IDとセット中のお供IDから都度判定する。
