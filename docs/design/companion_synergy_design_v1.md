...
- ホーム画面のお供パネル
  - 現在セット中のお供と現在のプレイヤー恐竜でシナジーが発動している場合のみ、短く `{shortLabel} 発動中` を表示。
- ホーム画面のお供選択モーダル
  - enabled の所持お供カードに `相性`、`共存シナジー名`、短い効果説明、`発動中/未発動` を表示。
  - enabled: false の将来予約お供は `相性: 未発見の恐竜` のみ表示。
- 研究画面のお供タブ 所持お供画面
  - 各所持お供行に相性恐竜、シナジー名、発動状態を表示。
...
CS05bでは、発動中の共存シナジーをPlaySceneの小型HUDとして表示する。

- 表示例: `共存: 王牙共鳴` / `ボス特化`
- 発動中のenabled synergyのみ表示する。
- 非シナジー構成、未解放予約シナジー、お供未セットでは表示しない。
- レベルアップ、進化BRANCH、ZERO通知、適応シナジー通知、ゲームパッド通知中は非表示にして重なりを避ける。
- 既存の適応シナジーHUDが出ている場合は、その下へ配置する。
- 詳細仕様は `docs/design/companion_synergy_hud_cs05b.md` に記録。
