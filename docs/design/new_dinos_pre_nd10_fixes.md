...
- ??Pt?DNA????????Pt? `researchPtConvertedToDna` ???????????
- ????????Pt 1 = DNA 5?
- ??????????????????????????????????????????????
- ?6???portrait 24?????????????BRANCH / ???? / HUD / ??????????????????
- ???????? / ?? / ???????????????????????????

## ND10pre UI Follow-up

The previous ND10pre cleanup is extended by `docs/design/new_dinos_home_research_ui_fix_nd10pre.md`.

Confirmed fixes:

- New six locked dino select cards use hero-derived silhouettes.
- Unknown research cards use DNA icon + numeric value.
- Unknown research tab exposes Spinosaurus plus all six new dino unlock cards through the scroll list.
- Home resource display is centered as a single DNA currency.
- Home lower panel is a non-interactive future-event placeholder.
- Home background is regenerated as a production dark sci-fi research facility illustration.
